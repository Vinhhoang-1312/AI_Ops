...
- LibreFace and Py-Feat both frame facial-expression analysis as face-first pipelines, so the app detects/crops faces before classification instead of classifying the full webcam frame.
- Multi-face support is implemented as detect-many + classify-each-crop, because the exported model is an expression classifier for one face crop at a time.
